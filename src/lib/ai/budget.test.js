...
const LABELS = { low: 'Low', medium: 'Medium', high: 'High', spendToday: '{amount} today' }
...
describe('budget', () => {
  ...
  it('omits empty slots from the formatted requests string, never shows zero', () => {
    ...
    const formatted = formatBudget(state, 'requests', slots, LABELS)
  ...
  it('falls back to the slot name when no labels are supplied', () => {
    ...
    expect(formatBudget(state, 'requests', { medium: MEDIUM_SLOT })).toBe('medium 2')
  })
  ...
  it('uses translated labels rather than hardcoded English', () => {
    ...
    const ptLabels = { low: 'Baixo', medium: 'Normal', high: 'Melhor', spendToday: '~{amount} hoje' }
    expect(formatBudget(state, 'requests', { medium: MEDIUM_SLOT }, ptLabels)).toBe('Normal 5')
  })
  ...
  it('formats estimated spend in tokens display mode', () => {
    let state = loadBudgetState()
    state = recordAiRequest(state, { slotName: 'medium', spend: 0.03 })
    state = recordAiRequest(state, { slotName: 'high', spend: 0.01 })

    const formatted = formatBudget(state, 'tokens', { medium: MEDIUM_SLOT, high: HIGH_SLOT }, LABELS)
    expect(formatted).toBe('~$0.04 today')
  })
})
