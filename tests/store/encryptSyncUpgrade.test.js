...
afterEach(async () => {
  const { closeDekStore } = await import('../../src/lib/crypto/keyStore.js')
  await closeDekStore()
  vi.unstubAllGlobals()
})
...
describe('enabling encryption while firebase sync is on', () => {
  it('encrypts the remote doc instead of leaving it plaintext', async () => {
    ...
    const { pushEnabledContainer } = await import('../../src/lib/firebase.js')
  ...
  it('lands in sync mode, not local-only mode', async () => {
    const { enableLocalEncryption } = await import('../../src/lib/crypto/localEnable.js')
    const { getEncMode, MODE_SYNC } = await import('../../src/lib/crypto/keyState.js')
  ...
  it('stays local-only when there is no firebase config', async () => {
    const { enableLocalEncryption } = await import('../../src/lib/crypto/localEnable.js')
    const { getEncMode, MODE_LOCAL } = await import('../../src/lib/crypto/keyState.js')
  ...
  it('does not enable encryption at all if the remote push fails', async () => {
    const { enableLocalEncryption, readLocalWraps } = await import('../../src/lib/crypto/localEnable.js')
    const { getEncMode, MODE_OFF } = await import('../../src/lib/crypto/keyState.js')
