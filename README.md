...
**Encryption.** Turning encryption on encrypts your data on the device, and if sync is connected the same key encrypts what goes to Firestore. Keys are derived with Argon2id and data is sealed with AES-GCM. You also get a 12-word recovery code as a second way in, transferable by QR, plus a passphrase hint, and you can change the passphrase, regenerate the recovery code, or rotate the underlying key. Backups can be exported encrypted too. If you lose both the passphrase and the recovery code, the data cannot be recovered by anyone.
